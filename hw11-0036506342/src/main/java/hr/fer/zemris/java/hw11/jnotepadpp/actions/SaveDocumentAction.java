...
/**
 * This class represents an action which saves the document to disk
 * @author Andrej
 *
 */
public class SaveDocumentAction extends AbstractAction{
	 ...
	 */
	public SaveDocumentAction(JNotepadPP frame, String key) {
		this.frame = frame;
		this.key = key;
	...
	public void actionPerformed(ActionEvent e) {
		if(frame.getModel().getCurrentDocument().getFilePath() != null) {
			frame.getModel().saveDocument(
					frame.getModel().getCurrentDocument(),
					frame.getModel().getCurrentDocument().getFilePath()
					);
		}
		else {
			Path openedFilePath;
